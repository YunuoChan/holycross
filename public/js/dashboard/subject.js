...
$('#coursePickerFilter-subject').on('change', function () {
    loadSubjectRecord();
});
...
$('#searchBtn-subject').on('click', function () {
    loadSubjectRecord();
});

$('#searchField-subject').on('blur', function () {
    loadSubjectRecord();
});
...
function loadSubjectRecord() {
    ...
    var keyword = $('#searchField-subject').val();
    // WEB SERVICE CALL 
    $.ajax({
        ...
        if (data.subjects.length > 0) {
            ...
            $('#subjectTable').append(showNoDataTableAvalable());
        }
    }).fail(function(error) {
...
function tableElement(subject) {
    var elm = BLANK;
        ...
        if (subject.status == 'ACT') {
            ...
            elm += '         <button type="button" class="btn btn-danger" id="trashSubject-'+ subject.id +'"><i class="fas fa-trash"></i></i>Delete</button> ';
        }
        elm += '        </div>'
        elm += '     </td> ';
        elm += ' </tr> ';
